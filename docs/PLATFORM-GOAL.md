...
| 2. Editing | User/org/document/tab-isolated drafts; recovery; expiration/logout; honest storage and save states; navigation protection; conflict comparison; idempotent mutations; browser failure tests | New and existing documents have scoped browser recovery, expiry, logout invalidation, and storage errors. Existing edits have explicit recovery comparison and conflict baseline acceptance. Initial automated Chromium failure tests added; full navigation/account-switch coverage and idempotency remain pending. See `DRAFT-RECOVERY.md` |
...
| EDIT-01 | High, fixed within documented scope | Original new-document form used `flexdocs_new_doc_draft` without user/org/tab identity. Replaced by scoped persistence and logout cleanup. | Storage unit tests and browser tab isolation checks cover the replacement. Full account-switch browser coverage remains pending. |
| EDIT-02 | High, partially fixed | Existing editor originally kept unsaved state only in memory. It now persists scoped edits and explicitly compares recovered copies. | Reload/network failure recovery and blocked-storage link navigation have browser tests. Programmatic/history navigation under storage failure remains pending. |
...
The baseline is an earlier reliability commit, not a published release. This does not establish compatibility with every historical schema or measure disaster-recovery RPO/RTO. CI now includes this same drill with full Git history so the pinned baseline is available. At that milestone, local verification was 171 tests passing, clean TypeScript, and zero lint errors (172 warnings).

## Evidence: browser editing recovery

The existing-document editor now persists unsaved fields with their server baseline, offers explicit comparison/recovery, and pauses autosave until an explicit save. Conflict responses show server and local versions and require acceptance of a new baseline before retrying. Autosave pause is independent of incidental metadata actions such as pinning.

Local validation: 182 unit tests, clean TypeScript, zero lint errors (172 existing warnings), and five Chromium scenarios covering network-save failure/reload recovery, independent tabs and document switching, storage failure/link protection, logout invalidation, and two-tab conflict resolution. The browser suite uses synthetic intercepted document responses; the existing live API suite remains the evidence for real server concurrency/authorization. CI now runs both suites. Full editing milestone coverage is still pending as listed in `DRAFT-RECOVERY.md`.
