...
3. **Document deletion is recoverable.** Single and bulk deletion move documents to owner-only Trash, retaining history, tags, and attachments. Restore requires editing permission and makes the document private while preserving its archive status. There is no automatic purge or permanent-delete API; storage grows until an explicit retention policy is implemented. See [FEATURE-WORKFLOWS.md](FEATURE-WORKFLOWS.md).
